...
public class PlatesMakerGuiGui extends SlothcraftModElements.ModElement {
	...
	private static class ContainerRegisterHandler {
		...
		public void registerContainer(RegistryEvent.Register<ContainerType<?>> event) {
			event.getRegistry().register(containerType.setRegistryName("plates_maker_gui"));
		}
	}

	@OnlyIn(Dist.CLIENT)
	public void initElements() {
		DeferredWorkQueue.runLater(() -> ScreenManager.registerFactory(containerType, PlatesMakerGuiGuiWindow::new));
	}
		...
		public GuiContainerMod(int id, PlayerInventory inv, PacketBuffer extraData) {
			...
			this.customSlots.put(1, this.addSlot(new SlotItemHandler(internal, 1, 79, 30) {
				...
				public boolean isItemValid(ItemStack stack) {
					return (PlateHammerItemItem.block == stack.getItem());
				}
			}));
	...
	private static void handleSlotAction(PlayerEntity entity, int slotID, int changeType, int meta, int x, int y, int z) {
		...
		if (slotID == 0 && changeType == 0) {
			...
			ItemMoverPlatesMakerProcedureProcedure.executeProcedure(Stream
